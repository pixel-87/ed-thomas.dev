...
In my opinion, the first point-doing one thing well-is the most relevant concerning vibe coding, seen in how LLMs handle high context scenarios.

LLMs thrive in small problem spaces. Huge codebases or ambiguous, broad goals for the final product often lead to a frustrating experience working against hallucinations, the agent fixes one bug only to create two more.

By keeping projects (and the context window) small, so that a piece of software only solves one problem, vibe coding does a much better job at implementing what you are imagining. Doing one problem, and doing it well.
...
AI allows for hyper-personalised, utility software to be made incredibly easily, there is no barrier to entry on needing to learn specific tech to solve a specific problem.

## My Example - A Browser Extension

I frequently do paid surveys on [prolific.com](https://www.prolific.com). These surveys payout in both pounds (£) and dollars ($).
...
This now has ~55 users on the chrome webstore 
